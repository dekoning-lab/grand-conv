...
// #define JDKLAB        1 // comment out this line to run normal codeML program
#define NS            7000
...
#ifdef JDKLAB
   void getSelectedBranches(char *line, char *opt, int firstCalled);
#endif
...
#ifdef JDKLAB
   char ctlf[96]="grand-conv.ctl", *pmodel, timestr[64];
#endif
...
*/
   ...
   if(argc == 2){
      if(strstr(argv[1], ".html")) 
         strcpy(com.htmlFileName, argv[1]);
      else
         strncpy(ctlf, argv[1], 95);
   }
   if(argc > 2){
      strncpy(ctlf, argv[1], 95); 
      if(strstr(argv[2], ".html")) strcpy(com.htmlFileName, argv[2]);
   }
#endif
...
int Forestry (FILE *fout)
{
   ...
   for(itree=0; ntree==-1||itree<ntree; itree++,iteration=1) {
      ...
      if(com.sspace < spaceming2(np)) {
         ...
         if((com.space=(double*)realloc(com.space,com.sspace))==NULL) {
            //printf("\ntrying to get %u bytes for ming2", com.sspace);
            error2("oom space");
         }
      }
...
#ifdef JDKLAB // read selected branches from input file
void getSelectedBranches(char *line, char *opt, int firstCalled){
   int start, end, index;
   ...
   index = firstCalled?0:1;
   while(tmp != NULL) {
      if(index > com.numOfSelectedBranchPairs*3) break;
      com.selectedBranchPairs[index] = atoi(tmp);
      if(firstCalled) com.selectedBranchPairs[index+2] = index/3;
      tmp = strtok(NULL, ",");
      index += 3;
   }
...
#ifdef JDKLAB
               case (37): getSelectedBranches(line, opt, 1); break;
               case (38): getSelectedBranches(line, opt, 0); break;
